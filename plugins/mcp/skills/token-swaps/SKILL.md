...
2. `get_swap_quote` for the wallet already in the session — the connected wallet, or the address the user gave; ask for an address only when neither exists, and say which wallet the quote was for. Amounts in main units; `kind: "sell"` spends an exact amount, `"buy"` receives one. Pass `slippagePct` to cap slippage; omitting accepts the backend's suggestion, which moves with market conditions.
...
- A quote was returned — rate, both amounts, slippage cap, route and the wallet it was made for — or the answer says which chain or token stopped it.
- Nothing has been signed or relayed unless the user handed over a signature.
